...
async function processConnectedInboxMessage(accountId, connectionId, payload = {}) {
  ...
  if (actionType === "deleted") {
    ...
    const labelId = await getOrCreateGmailLabelId(connection, labelName)
    await updateGmailMessageLabels(connection, message.id, [labelId], actionType === "validated" ? ["INBOX", "UNREAD"] : ["UNREAD"])
  }
  ...
  if (sharingEnabled && mailboxResource?.id) {
    upsertMailboxMessageCollaboration({
      ...
      lockOwnerAccountId: null,
      lockExpiresAt: null,
      assignmentState: actionType === "timed_out" ? "claimed" : actionType === "rejected" ? "new" : "completed",
      collaborationState: actionType === "timed_out" ? "awaiting_review" : actionType === "rejected" ? "available" : actionType,
  ...
  return {
    ...
    action,
    escalatedPriority,
    escalatedPriorityLabel: PRIORITY_LABELS[escalatedPriority] || message.analysis?.priorityLabel || "Normale",
    sentMessage: {
      id: sentMessage.id || "",
