...
const { isKnownStatsEventType } = require("./statsEventCatalog")
...
function initSchema(db) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS stats_events (
      ...
      generated_character_count INTEGER,
      quality_flag TEXT,
      error_code TEXT,
      error_message TEXT,
      metadata_json TEXT,
    ...
    CREATE INDEX IF NOT EXISTS idx_stats_events_event_timestamp ON stats_events(event_timestamp);
    CREATE INDEX IF NOT EXISTS idx_stats_events_environment ON stats_events(environment);
    CREATE INDEX IF NOT EXISTS idx_stats_events_user_id ON stats_events(user_id);
    CREATE INDEX IF NOT EXISTS idx_stats_events_account_id ON stats_events(account_id);
    CREATE INDEX IF NOT EXISTS idx_stats_events_session_id ON stats_events(session_id);
    CREATE INDEX IF NOT EXISTS idx_stats_events_event_type ON stats_events(event_type);
    CREATE INDEX IF NOT EXISTS idx_stats_events_status ON stats_events(status);
    CREATE INDEX IF NOT EXISTS idx_stats_events_workflow ON stats_events(workflow);
    CREATE INDEX IF NOT EXISTS idx_stats_events_provider_model ON stats_events(provider, model_name);
    CREATE INDEX IF NOT EXISTS idx_stats_events_user_timestamp ON stats_events(user_id, event_timestamp);
    CREATE INDEX IF NOT EXISTS idx_stats_events_account_timestamp ON stats_events(account_id, event_timestamp);
...
function migrateMailboxSharingSchemaIfNeeded(db) {
  ...
  if (!hasColumn(db, "provider_usage_events", "estimated_cost_amount")) {
    db.exec("ALTER TABLE provider_usage_events ADD COLUMN estimated_cost_amount REAL NOT NULL DEFAULT 0;")
    db.exec(`
      UPDATE provider_usage_events
      SET estimated_cost_amount = COALESCE(estimated_cost_cents, 0) / 100.0
      WHERE estimated_cost_amount IS NULL OR estimated_cost_amount = 0;
    `)
  }
}
...
function findOpenRequestByEmail(email) {
  const db = openDatabase()
  return db.prepare(`
    SELECT id, status, account_type, usage_mode, email, linked_account_id, planned_mailboxes
    FROM account_requests
    WHERE email = ?
      AND status IN ('pending', 'more_info_requested', 'approved')
    ORDER BY created_at DESC
...
function findOpenRequestByPlannedMailboxEmail(email) {
  const rows = listAccountRequests().filter((request) => (
    ["pending", "more_info_requested", "approved"].includes(request.status)
  ))
  const normalized = String(email || "").trim().toLowerCase()
  return rows.find((request) => (
    String(request.plannedMailboxes || request.planned_mailboxes || "")
      .split(/[\s,;]+/)
...
function findExistingMailboxEmail(email) {
  const db = openDatabase()
  const mailboxConnectionMatch = db.prepare(`
  ...
  if (mailboxConnectionMatch) {
    return mailboxConnectionMatch.mailbox_email
  }

  const mailboxResourceMatch = db.prepare(`
  ...
  return mailboxResourceMatch?.email_address || null
}
...
function createPendingAccountFromRequest(request) {
  ...
  const isAdminRequest = request.accountType === "administrateur"
  const account = {
    id: `${isAdminRequest ? "admin" : "acc"}-${Date.now()}`,
    created_at: request.createdAt,
    updated_at: now,
    status: "pending_activation",
    role: isAdminRequest ? "admin" : "user_standard",
    account_type: isAdminRequest ? "individuel" : request.accountType,
    usage_mode: request.usageMode,
    product_version: isAdminRequest ? "automatisation_poussee" : "base",
    first_name: request.firstName,
    last_name: request.lastName,
    email: request.email,
    phone: request.phone || "",
    organization_name: request.organizationName || "",
    password_hash: request.passwordHash || null,
    validated_by: null,
    validated_at: null,
...
function mapProviderUsageEventRow(row) {
  ...
  const estimatedCostAmount = row.estimated_cost_amount !== undefined && row.estimated_cost_amount !== null
    ? Number(row.estimated_cost_amount || 0)
    : Number(row.estimated_cost_cents || 0) / 100
...
function recordProviderUsageEvent(payload) {
  ...
  const estimatedCostAmount = payload.estimatedCostAmount !== undefined && payload.estimatedCostAmount !== null
    ? Number(payload.estimatedCostAmount || 0)
    : Number(payload.estimatedCostCents || 0) / 100
...
function getProviderUsageSummaryForAccount(accountId, options = {}) {
  ...
  const rows = db.prepare(`
    SELECT
      ...
      SUM(estimated_cost_amount) AS total_cost_amount,
  ...
  const providers = rows.map((row) => ({
    ...
    estimatedCostAmount: Number(row.total_cost_amount || 0),
    currency: row.currency || "EUR",
  ...
  return {
    ...
    totalEstimatedCostAmount: providers.reduce((sum, row) => sum + row.estimatedCostAmount, 0),
...
function normalizeStatsBoolean(value) {
  if (value === null || value === undefined || value === "") return null
  return value ? 1 : 0
}

function normalizeStatsInteger(value) {
  if (value === null || value === undefined || value === "") return null
...
function normalizeStatsReal(value) {
  if (value === null || value === undefined || value === "") return null
  const parsed = Number(value)
  if (!Number.isFinite(parsed)) return null
  return parsed
...
function sanitizeStatsEventPayload(payload = {}) {
  const eventType = String(payload.event_type || "").trim()
  if (!eventType) {
    throw new Error("event_type est obligatoire pour un evenement statistique.")
  }
  if (!isKnownStatsEventType(eventType)) {
    throw new Error(`event_type inconnu pour les statistiques : ${eventType}`)
  }

  const status = String(payload.status || "").trim() || "succeeded"
  const now = new Date().toISOString()
  const metadataJson = payload.metadata_json !== undefined
  ...
  return {
    ...
    event_timestamp: String(payload.event_timestamp || now),
    environment: String(payload.environment || process.env.APP_ENVIRONMENT || process.env.NODE_ENV || "prod").trim().toLowerCase(),
    event_type: eventType,
    status,
    success: normalizeStatsBoolean(payload.success),
    ...
    action_scope: payload.action_scope ? String(payload.action_scope) : null,
    message_id: payload.message_id ? String(payload.message_id) : null,
    attachment_count: normalizeStatsInteger(payload.attachment_count),
    source_language: payload.source_language ? String(payload.source_language) : null,
    target_language: payload.target_language ? String(payload.target_language) : null,
    automatic: normalizeStatsBoolean(payload.automatic),
    provider: payload.provider ? String(payload.provider) : null,
    model_name: payload.model_name ? String(payload.model_name) : null,
    model_label: payload.model_label ? String(payload.model_label) : null,
    model_kind: payload.model_kind ? String(payload.model_kind) : null,
    selection_mode: payload.selection_mode ? String(payload.selection_mode) : null,
    fallback_from_provider: payload.fallback_from_provider ? String(payload.fallback_from_provider) : null,
    fallback_from_model: payload.fallback_from_model ? String(payload.fallback_from_model) : null,
    fallback_reason: payload.fallback_reason ? String(payload.fallback_reason) : null,
    duration_ms: normalizeStatsInteger(payload.duration_ms),
    input_tokens: normalizeStatsInteger(payload.input_tokens),
    output_tokens: normalizeStatsInteger(payload.output_tokens),
    estimated_cost: normalizeStatsReal(payload.estimated_cost),
    generated_word_count: normalizeStatsInteger(payload.generated_word_count),
    generated_character_count: normalizeStatsInteger(payload.generated_character_count),
    quality_flag: payload.quality_flag ? String(payload.quality_flag) : null,
...
function recordStatsEvent(payload = {}) {
  const db = openDatabase()
  const event = sanitizeStatsEventPayload(payload)
...
function listStatsEvents(filters = {}) {
  const db = openDatabase()
  const clauses = []
  const params = {}

  if (filters.environment) {
    clauses.push("environment = @environment")
    params.environment = String(filters.environment)
  }
  if (filters.user_id) {
...
function resetStatsEventsForEnvironment(environment = "test") {
  const normalizedEnvironment = String(environment || "").trim().toLowerCase()
  if (normalizedEnvironment !== "test") {
    throw new Error("La remise a zero des evenements statistiques est reservee a l'environnement test.")
  }

  const db = openDatabase()
  const result = db.prepare("DELETE FROM stats_events WHERE environment = ?").run(normalizedEnvironment)
  return result.changes || 0
}


function resetStatsEventsForAccount(accountId) {
  const normalizedAccountId = String(accountId || "").trim()
  if (!normalizedAccountId) {
    throw new Error("Compte manquant pour la remise a zero des evenements statistiques.")
  }
  ...
  return {
    ...
    deletedCount: Number(before?.count || 0)
  }
}
...
function getMailboxMessageActionStats(accountId, connectionId) {
  ...
  for (const row of rows) {
    if (row.source_status === "validated") {
      stats.validated = Number(row.total || 0)
    } else if (row.source_status === "rejected") {
      stats.rejected = Number(row.total || 0)
    } else if (row.source_status === "deleted") {
...
module.exports = {
  ...
  recordStatsEvent,
  listStatsEvents,
  resetStatsEventsForEnvironment,
